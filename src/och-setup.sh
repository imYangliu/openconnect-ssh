#!/usr/bin/env bash
# Deprecated: this shell setup helper is kept for legacy package compatibility
# and shell tests. New `och setup` runs the Rust Ratatui/Crossterm wizard.
set -euo pipefail
...
och_setup_render_toml() {
  ...
  local route_mode="${OCH_ROUTES_MODE:-openconnect}"
  local proxy_section=""
  ...
  [[ -n "$routes" ]] && route_mode="extra"
  if [[ "${OCH_PROXY_ENABLED:-0}" == "1" ]]; then
    proxy_section="
[proxy]
...
och_setup_render_managed_ssh_config() {
  cat <<EOF
...
Host ${OCH_SSH_HOST}
  ...
  ProxyCommand $(och_setup_quote_ssh_config "$(och_setup_bin_path)") proxy-command %h %p
...
och_setup_validate_ssh_config() {
  local config_file="$1" host="$2" output
  if ! output="$(ssh -F "$config_file" -G "$host" 2>&1 >/dev/null)"; then
    echo "Error: generated SSH config failed validation: $output" >&2
    return 1
  fi
}
...
och_setup_write_managed_ssh_config() {
  local tmp_config
  tmp_config="$(mktemp "${TMPDIR:-/tmp}/och-managed-ssh.XXXXXX")"
  och_setup_render_managed_ssh_config >"$tmp_config"
  chmod 600 "$tmp_config"
  if ! och_setup_validate_ssh_config "$tmp_config" "$OCH_SSH_HOST"; then
    rm -f "$tmp_config"
    return 1
  fi

  install -d -m 0700 "$(dirname "$OCH_MANAGED_SSH_CONFIG")"
  mv "$tmp_config" "$OCH_MANAGED_SSH_CONFIG"
  chmod 600 "$OCH_MANAGED_SSH_CONFIG"
}

och_setup_ensure_include_line() {
  local include_line="Include ~/.ssh/och.config" tmp_config
  install -d -m 0700 "$(dirname "$OCH_MAIN_SSH_CONFIG")"
  if [[ -r "$OCH_MAIN_SSH_CONFIG" ]] && grep -Fxq "$include_line" "$OCH_MAIN_SSH_CONFIG"; then
    return 0
  fi
  tmp_config="$(mktemp "${TMPDIR:-/tmp}/och-main-ssh.XXXXXX")"
  printf '%s\n' "$include_line" >"$tmp_config"
  chmod 600 "$tmp_config"
  if ! och_setup_validate_ssh_config "$tmp_config" "__och_validation_probe__"; then
    rm -f "$tmp_config"
    return 1
  fi
  rm -f "$tmp_config"
...
och_setup_interactive() {
  ...
  local vpn_password=""
  vpn_password="$(och_setup_prompt_secret "VPN 密码（保存到 secrets.env，权限 0600）")"
