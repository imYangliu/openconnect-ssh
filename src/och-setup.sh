...
och_setup_probe_authgroups() {
  ...
  "$OPENCONNECT_BIN" "$host" -u "$user" --authenticate --non-inter 2>&1 |
...
och_setup_keychain_save_password() {
  ...
  "$OCH_SECURITY_BIN" delete-generic-password -s "$OCH_KEYCHAIN_SERVICE" -a "$account" >/dev/null 2>&1 || true
  "$OCH_SECURITY_BIN" add-generic-password -U -s "$OCH_KEYCHAIN_SERVICE" -a "$account" -w "$password" >/dev/null
}

och_setup_render_toml() {
  local routes="$1" prefix route_lines=""
  prefix="$(och_setup_brew_prefix)"
...
[vpn]
host = $(och_setup_quote_toml "${VPN_HOST:-}")
user = $(och_setup_quote_toml "${VPN_USER:-}")
auth_group = $(och_setup_quote_toml "${VPN_AUTHGROUP:-}")

[ssh]
host = $(och_setup_quote_toml "${DEFAULT_HOST:-}")
target_host = $(och_setup_quote_toml "${TARGET_HOST:-}")
user = $(och_setup_quote_toml "${TARGET_SSH_USER:-${USER:-}}")
port = $(och_setup_quote_toml "${TARGET_PORT:-22}")

[routes]
extra = [$route_lines]

[proxy]
local_host = $(och_setup_quote_toml "${PROXY_LOCAL_HOST:-127.0.0.1}")
local_port = $(och_setup_quote_toml "${PROXY_LOCAL_PORT:-7890}")
remote_port = $(och_setup_quote_toml "${PROXY_REMOTE_PORT:-7890}")

[paths]
och = $(och_setup_quote_toml "${OCH_PATH:-$prefix/bin/och}")
och_vpn = $(och_setup_quote_toml "${OCH_VPN_PATH:-$prefix/bin/och-vpn}")
askpass = $(och_setup_quote_toml "${OCH_ASKPASS_PATH:-$prefix/libexec/och/och-sudo-askpass.sh}")

[app]
language = $(och_setup_quote_toml "${APP_LANGUAGE:-system}")
...
Host ${DEFAULT_HOST}
  ...
  User ${TARGET_SSH_USER:-${USER:-}}
  Port ${TARGET_PORT:-22}
  ProxyCommand ${OCH_PATH:-$(och_setup_brew_prefix)/bin/och} --proxy-command %h %p
...
och_setup_interactive() {
  ...
  echo "OCH setup"
  VPN_HOST="$(och_setup_prompt "VPN 网关" "${VPN_HOST:-}")"
  VPN_USER="$(och_setup_prompt "VPN 用户" "${VPN_USER:-}")"
  local vpn_password=""
  vpn_password="$(och_setup_prompt_secret "VPN 密码（macOS 将保存到 Keychain）")"
  ...
  if [[ -n "$VPN_HOST" && -n "$VPN_USER" ]]; then
    ...
    while IFS= read -r group; do
      groups+=("$group")
    done < <(och_setup_probe_authgroups "$VPN_HOST" "$VPN_USER")
  fi
  VPN_AUTHGROUP="$(och_setup_select_authgroup "${groups[@]}")"
  VPN_AUTHGROUP="${VPN_AUTHGROUP:-$(och_setup_prompt "认证组（可留空）" "${VPN_AUTHGROUP:-}")}"
  ...
  if [[ -n "$selected" ]]; then
    ...
    IFS=$'\t' read -r TARGET_HOST TARGET_SSH_USER TARGET_PORT <<<"$resolved"
    DEFAULT_HOST="$(och_setup_managed_alias "$selected")"
  else
    DEFAULT_HOST="$(och_setup_prompt "托管 Host 别名" "${DEFAULT_HOST:-och-target}")"
    TARGET_HOST="$(och_setup_prompt "HostName/IP" "${TARGET_HOST:-}")"
    TARGET_SSH_USER="$(och_setup_prompt "SSH 用户" "${TARGET_SSH_USER:-${USER:-}}")"
    TARGET_PORT="$(och_setup_prompt "SSH 端口" "${TARGET_PORT:-22}")"
  fi

  local default_cidr route_cidr routes
  default_cidr="$(och_setup_default_cidr_for_host "$TARGET_HOST")"
  ...
  routes="$(och_setup_append_route "${MACOS_EXTRA_ROUTES:-${VPN_ROUTES:-}}" "$route_cidr")"

  och_setup_write_config "$routes"
  och_setup_keychain_save_password "$VPN_USER" "$vpn_password"
  och_setup_write_managed_ssh_config
  och_setup_ensure_include_line

  echo "已写入配置: $OCH_CONFIG_FILE"
  echo "已写入 SSH Host: $OCH_MANAGED_SSH_CONFIG"
  echo "默认连接: och ${DEFAULT_HOST}"
}
